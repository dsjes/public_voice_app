...
oid sha256:4ffca99f732518f8419f4042fa8be045cd086a8eca147b685c3678769f2d3ed4
size 51903
=======
...
with tab1:
    with st.form("my_form"):
        ...
        with left_col:
            start_now = st.date_input("**當期開始日期**",datetime.date(2023, 1, 1))

        with right_col:
            end_now = st.date_input("**當期結束日期**",datetime.date(2023, 1, 31))
    ...
    # 輿情總覽
    st.subheader("輿情總覽")
    df_品牌_情緒 = app_functions.find_品牌情緒(df_ptt,df_dcard,df_gmap,brand)
    fig = px.bar(df_品牌_情緒, x = "比例",y = "管道",color = "文章情緒",
                category_orders={"文章情緒": ["正向", "中性", "負向"]},
                 text="數量",
                color_discrete_map = {
                    "正向": "#A0C8AD",
                    "中性": "#FFE385",
    ...
    with col2:
        ...
        for i in range(len(gmap_name)):
            ...
            評價 = gmap_name["評價"][i]
            sep_line = "\n"
            list1.append(f'**{店點}** :  {sep_line} {rating:<6} {評價:>6} {sep_line}')
        nl = '\n'
    ...
    if (options == "PTT") or (options == "Dcard"):
        ...
        option_情緒 = st.radio("選擇情緒:",('正向情緒', '負向情緒',"中性情緒","全部"))
    ...
    # google map
        ...
        data_brand1_now = data_brand1_now[(data_brand1_now["review_time_日期"] >= start_choose) & (data_brand1_now["review_time_日期"] <= end_choose)]

        data_brand2_now = data[data["品牌"] == option_競品2]
        data_brand2_now = data_brand2_now[(data_brand2_now["review_time_日期"] >= start_choose) & (data_brand2_now["review_time_日期"] <= end_choose)]
        ...
        with col1:
            fig = px.scatter_mapbox(df_gmap聲量, size = '聲量',lat="lat", lon="lon", zoom = 6.15,width = 350,height = 500,color = '品牌',
                                ...
                                f"{brand}": "#F62929",
                                f"{option_競品1}": "#4D2929",
                                f"{option_競品2}": "#118CB3"},category_orders = {'品牌':[brand,option_競品1,option_競品2]})
            #carto-positron
        ...
        st.subheader("評論清單")
        option_情緒 = st.radio("選擇一種情緒:",('正向', '負向',"中性","全部"))
        if option_情緒 == "全部":
            data_品牌_now = data_now[data_now["品牌"] == brand]
...
with tab2:
    with st.form('my_form2'):
        ...
        st.write("**時間區間:**")
        with left_col:
            start_now = st.date_input("**開始時間**",datetime.date(2023, 1, 1))

        with right_col:
            end_now = st.date_input("**結束時間**",datetime.date(2023, 1, 31))
...
with tab3:
    ...
    df_ptt_thismonth = pd.read_sql(f"SELECT * FROM network_g.ptt_article  WHERE DATE(created_at) > '{start}' AND DATE(created_at) <= '{today}';",con = conn)
    df_ptt_thismonth["互動數"] = df_ptt_thismonth["dislike_count"] + df_ptt_thismonth["neutral_count"] +\
    df_ptt_thismonth["like_count"]+ df_ptt_thismonth["comment_count"]
    df_ptt_thismonth["日期"] = [str(i).split(" ")[0] for i in df_ptt_thismonth["created_at"]]
    board_list = df_ptt_thismonth["board"].unique()

    for i in range(len(df_ptt_thismonth["board"].unique())):
        if i == 0:
            data = df_ptt_thismonth.groupby("board").get_group(board_list[i]).sort_values("互動數",ascending = False)
            data.reset_index(inplace = True,drop = True)
...
>>>>>>> 1ff2f3ad45bba620b1fe0849b28555380f79b57e
